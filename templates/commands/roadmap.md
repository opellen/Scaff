...
6. If `$DocsDir/CONTEXT.md` exists AND the roadmap introduced new project-level context (principles, resources, processes), suggest declaratively: "Project context needs updating — `/scaff:context sync`." Otherwise say nothing — `scaff-flow` caps context-sync suggestions per session.
